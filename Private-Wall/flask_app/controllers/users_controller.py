...
def dashboard():
    if not "user_id" in session:
        return redirect('/')
    # we are storing the logged in user into session bc we want to use session to display the logged in user's first name 
    # & message they received based on their id 
    data = {
        'id': session['user_id']
    }
